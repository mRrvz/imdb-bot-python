...
def send_msg(bot, obj_msg, msg, *argv):
    ...
    list(map(lambda x: markup.add(x), list(argv)))
    bot.send_message(obj_msg.chat.id, msg, reply_markup=markup, parse_mode="markdown")


def change_lang(bot, message, user):
    if message.text not in lang:
        bot.send_message(message.chat.id, "❗ Выберите корректный язык из списка / Select available language.")
    else:
...
def find_actor(bot, message, user):
    ...
    bot.send_message(message.chat.id, dict_phr[user.language], reply_markup=hide_markup)
...
def pars_ids(msg, user, act1, act2):
    ...
    fin_msg = dict_phr[user.language][0] + act1 + dict_phr[user.language][1] + act2 + "\n\n"
    fin_msg += dict_phr[user.language][2] + str(arr_msg[0]) + dict_phr[user.language][3] + "\n\n"
...
def find_link(bot, message, user, act1, act2):
    bot.send_chat_action(message.chat.id, "typing")
    res = subprocess.check_output(["/root/IMDBbot/IMDBgame/./game.out", act1, act2])

    dict_phr = messages["try_more"]
    list_phr = dict_phr[user.language]
    msg = pars_ids(res, user, act1, act2)
    send_msg(bot, message, msg, *[list_phr[0] + act1, list_phr[0] + act2, list_phr[1]])
